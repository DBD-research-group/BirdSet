...
class EventDecoding:
    ...
    def _time_shifting(self, start, end, total_duration):
        ...
        max_start_interval = round(new_end_time - self.extracted_interval,2)
        random_start = round(random.uniform(new_start_time, max_start_interval),2) 
        random_end = round(random_start + self.extracted_interval,2)
