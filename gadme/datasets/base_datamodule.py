...
class BaseDataModule(L.LightningDataModule):
    ...
    def __init__(
            ...
            val_split,
            transforms=None, 
            columns=["input_values", "ebird_codes"]
    ):
    ...
    def _preprocess_function(self, batch):
        ...
        inputs = self.feature_extractor(
            audio_arrays,
            sampling_rate=self.feature_extractor,
            padding=True,
            max_length=self.feature_exatractor*5,
            truncation=True,
            return_tensors="pt"
        )
        return inputs
...
class BaseDataModuleC(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        if not self.train_dataset and not self.val_dataset:
            if stage == "fit":
                logging.info("fit")
                self.train_dataset = load_from_disk(os.path.join(self.data_path,"train"))
