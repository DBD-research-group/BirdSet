...
for i in "${!dnames[@]}"; do
  ...
  for experiment in "$@"; do
    ...
    python birdset/train.py --multirun experiment="local/finetune/BEANS/$experiment" seed=1,2,3 logger.wandb.group="results_finetuneX" trainer.devices=[3] trainer.min_epochs=5 trainer.max_epochs=30 datamodule.dataset.dataset_name=$dname datamodule.dataset.hf_path="DBD-research-group/$dname" datamodule.dataset.n_classes=$dclass
  done
done