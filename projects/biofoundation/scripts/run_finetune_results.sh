...
for i in "${!dnames[@]}"; do
  ...
  for experiment in "$@"; do
    ...
    python birdset/train.py --multirun experiment="local/finetune/BEANS/$experiment" seed=1,2,3 logger.wandb.group="results_finetune1" trainer.devices=[1] trainer.min_epochs=25 trainer.max_epochs=80 datamodule.dataset.dataset_name=$dname datamodule.dataset.hf_path="DBD-research-group/$dname" datamodule.dataset.n_classes=$dclass
  done
done