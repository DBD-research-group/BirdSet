...
class ASTModel(nn.Module):
    ...
    def forward(self, input_values, task='ft_avgtok', cluster=True, mask_patch=400, labels=None):
        ...
        if task == 'ft_avgtok':
            return self.finetuningavgtok(input_values)
        # alternatively, use the [cls] token output as clip-level representation.
        elif task == 'ft_cls':
            return self.finetuningcls(input_values)
        ...
        # pretraining, masked patch reconstruction (generative objective)
            ...
            return self.mpc(input_values, mask_patch=mask_patch, cluster=cluster, show_mask=True)
        else:
